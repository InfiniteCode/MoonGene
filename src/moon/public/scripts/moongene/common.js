...
function PDFTarget(target)
{
   // debugger;
	...
	form.find('input').val(target.html());
	form.submit();
}
...
$(function()
{
    ...
    $("#pricing-support").click(function() {
        $('#modal-support').modal();
        return false;
    });

    $("#modal-support-send").click(function() {
        $.post($("#modal-support-send").data("send-url"), $("#modal-support-form").serialize(), function(data) {
            if(data.code == "0")
